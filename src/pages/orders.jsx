        ...
        <span className="font-medium">Back to Previous</span>
                ...
                <div className="mb-8">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-secondary/70">Track and manage all your purchases</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                        <button
                            ...
                            className={`px-4 py-2 rounded-full text-sm font-medium transition ${filter === "all" ? "bg-accent text-white" : "bg-white text-secondary border border-secondary/10 hover:bg-accent/10"}`}
                        >
                            All Orders
                        </button>
                        <button
                            onClick={() => setFilter("pending")}
                            className={`px-4 py-2 rounded-full text-sm font-medium transition ${filter === "pending" ? "bg-yellow-100 text-yellow-800" : "bg-white text-secondary border border-secondary/10 hover:bg-yellow-50"}`}
                        >
                            Pending
                        </button>
                        <button
                            onClick={() => setFilter("processing")}
                            className={`px-4 py-2 rounded-full text-sm font-medium transition ${filter === "processing" ? "bg-blue-100 text-blue-800" : "bg-white text-secondary border border-secondary/10 hover:bg-blue-50"}`}
                        >
                            Processing
                        </button>
                        <button
                            onClick={() => setFilter("shipped")}
                            className={`px-4 py-2 rounded-full text-sm font-medium transition ${filter === "shipped" ? "bg-indigo-100 text-indigo-800" : "bg-white text-secondary border border-secondary/10 hover:bg-indigo-50"}`}
                        >
                            Shipped
                        </button>
                        <button
                            onClick={() => setFilter("completed")}
                            className={`px-4 py-2 rounded-full text-sm font-medium transition ${filter === "completed" ? "bg-green-100 text-green-800" : "bg-white text-secondary border border-secondary/10 hover:bg-green-50"}`}
                        >
                            Completed
                        </button>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-secondary/10">
                    {filteredOrders.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-lg font-semibold text-secondary mb-2">No orders found</h3>
                            <p className="text-secondary/70 mb-4">You haven't placed any orders yet</p>
                            <a 
                                href="/" 
                                ...
                                >
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-2">
                                                ...
                                                <div>
                                                    <h3 className="font-bold text-secondary">Order #{order.orderID}</h3>
                                    ...
                                    {selectedOrder?._id === order._id && (
                                        <div className="mt-6 pt-6 border-t border-secondary/10">
                                            <h4 className="font-semibold text-secondary mb-4">Order Items</h4>
                                            <div className="space-y-3">
                                                {order.items?.map((item, index) => (
                                                    <div key={index} className="flex items-center gap-4 p-3 bg-primary/30 rounded-lg">
                                                        <img 
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-sm text-secondary/70">Quantity: {item.quantity}</p>
                                                        </div>
                                                        <div className="ml-auto text-right">
                                                            <p className="font-medium text-secondary">LKR {item.price.toFixed(2)}</p>
                                                            <p className="text-sm text-secondary/70">Total: LKR {(item.price * item.quantity).toFixed(2)}</p>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                                                <div className="bg-primary/30 p-4 rounded-lg">
                                                    <h5 className="font-semibold text-secondary mb-2">Shipping Address</h5>
                                                    <p className="text-sm text-secondary/70">
                                                ...
                                                <div className="bg-primary/30 p-4 rounded-lg">
                                                    <h5 className="font-semibold text-secondary mb-2">Payment Method</h5>
                                                    <p className="text-sm text-secondary/70">{order.paymentMethod}</p>
                                                    <p className="text-sm text-green-600 font-medium mt-1">
                                                        {order.paymentStatus === 'paid' ? '✓ Paid' : 'Pending'}
                                                    </p>
                                                </div>
                                                <div className="bg-primary/30 p-4 rounded-lg">
                                                    <h5 className="font-semibold text-secondary mb-2">Order Summary</h5>
                                                    <div className="space-y-1 text-sm">
                                                        <div className="flex justify-between">
                                                            <span>Subtotal</span>
                                                            <span>LKR {order.subtotal?.toFixed(2) || order.total.toFixed(2)}</span>
                                                        </div>
                                                        <div className="flex justify-between">
                                                            <span>Shipping</span>
                                                            <span>LKR {order.shippingFee?.toFixed(2) || '0.00'}</span>
                                                        </div>
                ...
                <div className="mt-8 bg-gradient-to-r from-accent/10 to-secondary/10 rounded-2xl p-6">
                    <div className="flex flex-col md:flex-row items-center justify-between">
                        <div>
                            <h3 className="text-xl font-bold text-secondary mb-2">Need help with your orders?</h3>
                            <p className="text-secondary/70">Contact our customer support for assistance</p>
                        </div>
                        <a 
                            href="/contact" 
