...
export default function DashboardHome() {
  ...
  useEffect(() => {
    ...
    async function fetchStats() {
      try {
        const [ordersRes, usersRes, messagesRes, productsRes] =
          await Promise.all([
        ...
        setTopProducts(
          [...productsRes.data].sort((a, b) => b.sold - b.sold).slice(0, 5)
        );
        setRecentOrders(
          [...ordersRes.data]
    ...
    <div className="p-6 w-full space-y-6 bg-primary min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {Object.entries(stats).map(([key, value]) => (
          <div
            key={key}
            className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border border-secondary/10"
          >
            ...
            <h3 className="text-sm font-medium text-secondary/70 uppercase tracking-wider">
              {key}
            </h3>
            <p className="text-3xl font-bold text-secondary mt-2">{value}</p>
            <div className="mt-4 pt-4 border-t border-secondary/10">
              <span className="text-xs text-secondary/60">
                +{Math.floor(value * 0.12)} from last month
              </span>
            </div>
          </div>
        ))}
          ...
          <div className="space-y-3">
            {lowStockProducts.length > 0 ? (
              lowStockProducts.map((p) => (
                <div
                  key={p._id}
                  className="flex items-center justify-between p-3 hover:bg-accent/5 rounded-xl transition-all group"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-medium text-secondary group-hover:text-accent">
                        {p.name}
                      </h4>
                  ...
                  <span className="text-accent font-bold text-lg bg-accent/10 px-3 py-1 rounded-full">
                    {p.stock}
                  </span>
                </div>
              ))
          ...
          <div className="space-y-3">
            {topProducts.length > 0 ? (
              topProducts.map((p, i) => (
                <div
                  key={p._id}
                  className="flex items-center justify-between p-3 hover:bg-yellow-50 rounded-xl transition-all group"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-medium text-secondary group-hover:text-accent truncate max-w-[200px]">
                        {p.name}
                      </h4>
                      <p className="text-xs text-secondary/60">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-secondary text-white">
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider rounded-l-lg">
                    Order ID
                  </th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider rounded-r-lg">
                    Status
                  </th>
                </tr>
              </thead>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 border border-secondary/10">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className="bg-secondary text-white">
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider w-1/3">
                    Name
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider w-2/5">
                    Email
                  </th>
              ...
              <tbody className="divide-y divide-secondary/10">
                {recentUsers.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-secondary/80 truncate">
                      {u.email}
                    </td>
                    <td className="px-4 py-3">
                      <span
