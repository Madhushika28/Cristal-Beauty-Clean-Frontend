...
export default function AdminFeedbackPage() {
    ...
    const deleteFeedback = async (feedbackId) => {
        ...
        try {
            await axios.delete(`${import.meta.env.VITE_API_URL}/api/feedback/${feedbackId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
        ...
        <div className="w-full min-h-full p-6">
            <div className="rounded-2xl border border-secondary/10 bg-primary shadow-sm">
                ...
                <div className="flex items-center justify-between gap-4 border-b border-secondary/10 px-6 py-4">
                    <h1 className="text-lg font-semibold text-secondary">Customer Feedback</h1>
                    ...
                    <table className="w-full min-w-[1000px] text-left">
                        <thead className="bg-secondary text-white">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-xs font-semibold uppercase">Rating</th>
                                <th className="px-4 py-3 text-xs font-semibold uppercase">Review</th>
                        ...
                        <tbody className="divide-y divide-secondary/10">
                            {feedbacks.map(fb => (
                                <tr key={fb._id} className="hover:bg-accent/5 transition-colors">
                                    {/* Product Info */}
...
<td className="px-4 py-3">
    <div className="flex items-center gap-3">
        <img src={fb.userImage} alt={fb.userName} className="w-8 h-8 rounded-full" />
        <div>
            ...
            <div className="text-xs text-gray-500">{fb.userEmail}</div>
                                    ...
                                    <td className="px-4 py-3 max-w-xs">
                                        <div className="text-sm line-clamp-2">{fb.review || "No review text"}</div>
                                        {fb.images?.length > 0 && (
                                            <div className="text-xs text-gray-500 mt-1">{fb.images.length} photo{fb.images.length !== 1 && "s"}</div>
                                        )}
                                    </td>
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex items-center justify-center gap-2">
                                            ...
                                            <button
                                                onClick={() => window.open(`/overview/${fb.productID}`, "_blank")}
