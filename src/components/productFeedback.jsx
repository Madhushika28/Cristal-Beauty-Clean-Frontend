...
export default function ProductFeedback({ productID }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      // Load stats
      const statsRes = await axios.get(`${import.meta.env.VITE_API_URL}/api/feedback/product/${productID}/stats`);
      setStats(statsRes.data);

      // Load feedbacks
      const feedbackRes = await axios.get(`${import.meta.env.VITE_API_URL}/api/feedback/product/${productID}`);
  ...
  const handleFeedbackSuccess = async (newFeedback) => {
    try {
      ...
      // Optimistically update user feedback
      setUserFeedback(newFeedback);
      
      // Reload stats and feedbacks immediately
      await loadData();
  ...
  try {
    // Optimistic update - immediately show the new count
    const updatedFeedbacks = feedbacks.map(fb => 
      fb._id === feedbackId 
        ? { ...fb, helpfulCount: currentCount + 1 }
        : fb
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left Column - Statistics and Reviews */}
        <div className="lg:col-span-2 space-y-6">
          {/* Statistics Card */}
          {stats && (
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-gray-900">Customer Reviews Summary</h3>
                {refreshing && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <RatingStars rating={stats.averageRating || 0} size="text-xl" showNumber={false} />
                      <p className="text-sm text-gray-500 mt-2">
                        {stats.totalRatings || 0} ratings • {stats.totalReviews || 0} reviews
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="font-semibold text-gray-700">Rating Breakdown</h4>
                ...
                {/* Verified Purchases & Additional Info */}
                <div className="space-y-6">
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-5 border border-green-100">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-semibold text-gray-800">Verified Purchases</h4>
                        <p className="text-sm text-gray-600 mt-1">Reviews from confirmed buyers</p>
                      </div>
                      <div className="bg-white rounded-full p-3 shadow-sm">
                        <span className="text-2xl font-bold text-emerald-600">{stats.verifiedPurchases || 0}</span>
                      </div>
                    </div>
                    {stats.verifiedPurchases > 0 && (
                      <div className="mt-4 text-xs text-emerald-600">
                        ✓ These reviews are from customers who purchased this product
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-blue-50 rounded-lg p-4 text-center border border-blue-100">
                      <div className="text-2xl font-bold text-blue-600">{stats.totalRatings || 0}</div>
                      <div className="text-sm text-blue-700 font-medium mt-1">Total Ratings</div>
                    </div>
                    <div className="bg-purple-50 rounded-lg p-4 text-center border border-purple-100">
                      <div className="text-2xl font-bold text-purple-600">{stats.totalReviews || 0}</div>
                      <div className="text-sm text-purple-700 font-medium mt-1">Written Reviews</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border-2 border-blue-200">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <h4 className="font-semibold text-gray-900">{userFeedback.userName}</h4>
                      <div className="flex items-center gap-2 mt-1">
                        <RatingStars rating={userFeedback.rating} size="text-sm" showNumber={false} />
                ...
                {userFeedback.review && (
                  <p className="mt-4 text-gray-700 leading-relaxed">{userFeedback.review}</p>
                )}
            ...
            {/* Other Reviews */}
            {feedbacks.filter(fb => !userFeedback || fb._id !== userFeedback._id).length > 0 ? (
              <div className="space-y-4">
                {feedbacks
                  .filter(fb => !userFeedback || fb._id !== userFeedback._id)
                  .map((feedback, index) => (
                    <div key={index} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:border-blue-200 transition-colors">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          <img 
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <RatingStars rating={feedback.rating} size="text-sm" showNumber={false} />
                              <span className="text-xs text-gray-500">
                      ...
                      {feedback.review && (
                        <p className="mt-4 text-gray-700 leading-relaxed">{feedback.review}</p>
                      )}
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
                        ...
                        <button 
  onClick={() => handleHelpfulClick(feedback._id, feedback.helpfulCount || 0)}
  ...
  disabled={JSON.parse(localStorage.getItem('likedFeedbacks') || '[]').includes(feedback._id)}
>
  ...
  Helpful ({feedback.helpfulCount || 0})
        ...
        {/* Right Column - Feedback Form and Guidelines */}
        <div className="space-y-6">
          {/* Feedback Form */}
          <FeedbackForm 
            productID={productID}
